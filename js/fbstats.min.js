...
c=$("#logintext"),e=b.children("img");b.show();c.text("Logging in to Facebook");FB.init({appId:a,xfbml:!1,cookie:!0});FB.getLoginStatus(function(a){switch(a.status){case "connected":c.text("Checking permissions");user=a.authResponse;checkPerms();break;default:c.text("Login to Facebook"),e.hide(),b.click(login)}});$("#threadcount").parent().resizable();$("#threadtime").parent().resizable()},checkPerms=function(){FB.api("/me/permissions",function(a){a.data[0].read_mailbox?start():(console.log(a.data),
$("#logintext").text("Could not access message data"),$("#loginbutton>img").hide())})},log=function(a){console.groupCollapsed(a.callee.name);for(var b in a)console.log(a[b]);console.trace();console.groupEnd()},mapTimestampsToDays=function(a){var b={},c=new Date(0),e=new Date(a[0]);e.setHours(0);e.setMinutes(0);e.setSeconds(0);e.setMilliseconds(0);switch(grouping){case "weekly":e.setDate(e.getDate()-e.getDay());break;case "monthly":e.setDate(1)}for(var d=0;d<a.length;d++)(new Date(a[d])).getTime()<
e.getTime()?b[c.getTime()]++:(c=new Date(e),b[c.getTime()]=0,e.addInterval(1));a=[];for(c=0;c<smooth;c++){var e={},f=d=0,h=0,g;for(g in b)h=f,f=d,d=g,0!=f&&(e[f]=0==h?(b[f]+b[d])/3:(b[f]+b[d]+b[h])/3);e[d]=(b[d]+b[f])/3;b=e}for(g in b)a.push([g,b[g]]);return a};FB.fql=function(a,b){log(arguments);FB.api({method:"fql.query",query:a},b)};Storage.prototype.setObject=function(a,b){this.setItem(a,JSON.stringify(b))};Storage.prototype.getObject=function(a){return(a=this.getItem(a))&&JSON.parse(a)};var Statistics=
{maxThreadCount:20,threads:[],reducedThreads:[],save:function(){localStorage.setItem("lastUpdate",Statistics.lastUpdate);0==Statistics.lastUpdate&&(Statistics.threads=[]);localStorage.setObject("threads",Statistics.threads)},load:function(){var a=localStorage.getItem("lastUpdate");if(!a||Date.now()-parseInt(a,10)>1E3*cacheTime)return!1;Statistics.threads=localStorage.getObject("threads");return!0},countThreads:function(a){a=a||0;FB.fql("select participants,num_messages,thread_id from unified_thread where folder='inbox' LIMIT "+
threadGetLimit+" OFFSET "+a,function(b){if($.isArray(b)){for(var c=0;c<b.length;c++)b[c].num_messages<MIN_THREAD_MSG_COUNT||Statistics.threads.push(new Thread(b[c]));$("#threadload").text("Getting thread "+Statistics.threads.length);0==b.length?(Statistics.threads.sort(function(a,c){return c.count-a.count}),Statistics.lastUpdate=Date.now(),Statistics.graphThreads()):Statistics.countThreads(a+b.length)}else $("#threadload").text("Error "+b.error_code+": "+b.error_msg),console.log("Error: ",b)})},graphThreads:function(){Statistics.reducedThreads=
...
e$$15;}"undefined"!=typeof console&&(console.olog="undefined"!=typeof console.log?console.log:function(){});console.log=function(a){console.olog(a);$("#debugDiv").append("<p>"+a+"</p>")};console.error=console.debug=console.info=console.log;
