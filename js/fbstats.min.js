...
0;c<b.length;c++)b[c].num_messages<MIN_THREAD_MSG_COUNT||Statistics.threads.push(new Thread(b[c]));$("#threadload").text("Getting thread "+Statistics.threads.length);0==b.length?(Statistics.threads.sort(function(a,b){return b.count-a.count}),Statistics.lastUpdate=Date.now(),Statistics.graphThreads()):Statistics.countThreads(a+b.length)}else $("#threadload").text("Error "+b.error_code+": "+b.error_msg),console.log("Error: ",b)})},graphThreads:function(){Statistics.reducedThreads=[];for(var a=0,b=0;b<
...
grouping?this.setMonth(this.getMonth()+a):"weekly"==grouping?this.setDate(this.getDate()+7*a):this.setDate(this.getDate()+a)};$(function(){var a=$("#loginbutton");$("#logintext");a.children("img");a.hide();$("#appidform").submit(function(){event.preventDefault();init($("#appidinput").val());$(this).hide()});var a=$("#scaleselect").change(function(){scale=scales[$(this).val()];Statistics.graphThreads();Statistics.graphMessages()}),b;for(b in scales)$("<option/>").text(b).appendTo(a);$("#groupingselect").change(function(){grouping=
$(this).val();Statistics.graphThreads();Statistics.graphMessages()});$("#threadcountinput").change(function(){var a=$(this).val()||15;3>a&&(a=3);50<a&&(a=50);$(this).val(a);Statistics.maxThreadCount=a;Statistics.graphThreads()});$(window).on("beforeunload",function(){Statistics.lastUpdate&&0<Statistics.threads.length&&Statistics.save()})})}catch(e$$14){throw $(".errormessage").append(e$$14).fadeIn(),e$$14;};
